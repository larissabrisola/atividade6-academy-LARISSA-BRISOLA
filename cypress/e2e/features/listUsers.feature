...
Para poder visualizar as informações 
   ...
   Scenario: Listar usuários com sucesso
      ...
      Then a lista de usuários será exibida 
   Scenario: Devem exibir opções para exibir detalhes ou excluir usuário 
      When existir usuários cadastrados 
      Then cada usuário deve possuir as opções de ver detalhes e excluir 
   ...
   Scenario: Deve ser possível avançar entre as páginas da lista de usuários
      ...
      Then devo ser capaz ir para a proxima página 
   Scenario: Deve ser possível voltar para a página anterior
      When existir mais que 6 usuários cadastrados 
      And clicar no botão de próximo 
      Then devo ser capaz de voltar a página Anterior 
